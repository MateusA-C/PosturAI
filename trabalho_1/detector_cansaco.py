...
from criacao_db import Usuario,Registro,session
from basededadosalchemy import GerenciadorBanco
...
class DetectorCansaco(DetectorOlhosEBoca):
    ...
    def __init__(self,email_usuario):
        ...
        self.meu_gerenciador = GerenciadorBanco()
        self.email_usuario = email_usuario
        self.usuario = self.session.query(Usuario).filter_by(email = self.email_usuario).first() #Coleta o usuário o qual eu estou querendo detectar dentro do banco de dados
        self.registro = Registro(usuario_id=self.usuario.id)
        self.session.add(self.registro)
        self.session.commit()
