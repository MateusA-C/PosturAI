...
from detector_cansaco import DetectorCansaco
...
class Principal:
    ...
    def _criar_conteudo_principal(self):
        ...
        self.btn_monitorar = ctk.CTkButton(self.content,
    ...
    def _iniciar_visao_computacional(self):
        ...
        email_usuario = self.controller.usuario_logado_email

        if not email_usuario:
            messagebox.showerror("Erro", "Sessão inválida. Por favor, faça login novamente.")
            return
        
        if self.meu_detector is None:
            self.lbl_status.configure(text="Iniciando câmera...", text_color="orange")
            self.parent_frame.update()
            self.meu_detector = DetectorCansaco(email_usuario)

        print(f"Iniciando monitoramento para: {email_usuario}")

        self.lbl_status.configure(text="Monitoramento ATIVO, pressione ESC para parar", text_color="#10b981")
        self.parent_frame.update()
        self.meu_detector.loop_captura()
